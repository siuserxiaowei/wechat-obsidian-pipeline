...
# === 配置: 监控的群列表 ===
declare -a TARGETS=(
  "43225648899@chatroom|星球AI实战圈"
  "1234567890@chatroom|天策成长团（白盒内参）"
  "56150811210@chatroom|AI编程-最小化商业产品"
)
...
process_group() {
  ...
  # 2b) 数据质量门槛: 太薄不出报告(AI 会瞎编)
  local count
  count=$(jq -r '.meta.total_count // 0' "$workdir/stats.json")
  local min_msgs="${MIN_MESSAGES:-30}"
  if [ "$count" -lt "$min_msgs" ]; then
    echo "  💤 当天只有 $count 条消息 (< $min_msgs 阈值), 不出日报"
    echo "     提示: export MIN_MESSAGES=10 可放宽门槛"
    return 1
  fi
